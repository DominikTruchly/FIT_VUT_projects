...
AutomatState getState(AutomatState currentState, char in)
{
    ...
    case Next:
    case Error:
        //fprintf(stderr, "Error while generating token on line %d\n", lineNumber);
        exit(1);
    ...
    case StringA:
        if (in > 31 && in != 36 && in != 34 && !isbackslash(in)){
            return StringA;
        }
        if (isbackslash(in)){
            return Backslash;
...
Lexeme makeLexeme(AutomatState finalState, char *Token){
    switch (finalState)
    {
        ...
        case Next: 
            fprintf(stderr, "Invalid lexeme generator");
            exit(1);
        default: 
            fprintf(stderr, "Invalid lexeme generator"); 
            exit(1);
